...
import { searchCustomers as searchCustomersMx } from "@/api/gatekeeper";
...
type ResolveExistingResult = Awaited<ReturnType<typeof resolveExisting>>;

interface PipelineCustomerResult {
  customerKey: string;
  fullName: string;
  phones?: string[];
}
...
const mapPipelineCustomersToResponse = (
  customers: PipelineCustomerResult[]
): ResolveExistingResult => {
  const normalizedRecords = customers.map((customer) => ({
...
const runNameOnlyLookup = async (
  firstName?: string,
  lastName?: string
): Promise<ResolveExistingResult> => {
  const query: { firstName?: string; lastName?: string } = {};
  if (firstName) {
  ...
  const response = (await searchCustomersMx(query)) as PipelineCustomerResult[];
  return mapPipelineCustomersToResponse(response || []);
};
...
const Gatekeeper1Form: React.FC = () => {
  ...
  const triggerExistingLookup = async (context: 'search' | 'existing' | 'newPartial') => {
    ...
    try {
      ...
      const cleanedPhone = normalizePhone(phone);
      const hasValidPhone = cleanedPhone.length >= MIN_PHONE_DIGITS;
      const usableFirstName = trimmedFirstName.length >= MIN_NAME_CHARS ? trimmedFirstName : '';
      const usableLastName = trimmedLastName.length >= MIN_NAME_CHARS ? trimmedLastName : '';
      const hasUsableName = usableFirstName !== '' || usableLastName !== '';

      if (!hasValidPhone && !hasUsableName) {
        setMessage({
          type: 'error',
          text: 'Inserisci almeno 2 lettere per Nome o Cognome per poter cercare senza telefono.',
        });
        return;
      }

      const result = hasValidPhone
        ? await resolveExisting({
            firstName: trimmedFirstName || undefined,
            lastName: trimmedLastName || undefined,
            phone: cleanedPhone,
          })
        : await runNameOnlyLookup(usableFirstName || undefined, usableLastName || undefined);
  ...
  const handleOpenModule = async () => {
    ...
    setLoading(true);
    try {
      const result = await checkDuplicate({ firstName: trimmedFirstName, lastName: trimmedLastName, phone: cleanedPhone });
  ...
  const handleResolveExistingResult = (
    ...
    context: 'search' | 'existing' | 'newPartial' = 'existing'
  ) => {
    const cleanedPhone = normalizePhone(phone);
